...
const SOURCES: [SourceDefinition; 4] = [
    SourceDefinition {
...
pub(crate) async fn put_preferences(
    ...
    sqlx::query(
        "UPDATE holiday_preferences SET state_code = $1, city_code = $2, include_national = $3, \
         include_state = $4, include_municipal = $5, include_commemorative = $6, \
...
pub(crate) async fn list_holidays(
    ...
    Ok(Json(HolidayRangeResponse { holidays, coverage }))
}
...
async fn load_preferences(state: &AppState) -> Result<HolidayPreferences, ApiError> {
    let row = sqlx::query(
        "SELECT trim(state_code) AS state_code, city_code, include_national, include_state, \
         include_municipal, include_commemorative, revision, \
...
async fn ensure_year(state: &AppState, year: i32) {
    let [national, state_result, municipal, commemorative] = SOURCES;
    let ((), (), (), ()) = tokio::join!(
    ...
    for raw in raw_entries {
        if raw.tipo != source.expected_type {
            return Err(format!(
                ...
                source.name, source.expected_type, raw.tipo
            ));
        }
        ...
        let state_code = match source.scope {
            "state" | "municipal" => normalize_source_state(raw.uf.as_deref(), source.name)?,
            _ => None,
        };
        let city_code = if source.scope == "municipal" {
            Some(
                raw.codigo_ibge
